...
public:
    ...
    static bool IsKeyPressed(Key key) { return ms_Pressed[(uint32_t)key]; }
    static bool WasKeyReleased(Key key) { return ms_WasReleased[(uint32_t)key]; }
    static bool WasKeyPressed(Key key) { return ms_WasPressed[(uint32_t)key]; }
...
private:
    ...
    static void ProcessKeyUp(uint32_t wParam)
    {
        ms_Pressed[wParam] = false;
        ms_WasReleased[wParam] = true;
    }

    static void ProcessKeyDown(uint32_t wParam)
    {
        ms_Pressed[wParam] = true;
        ms_WasPressed[wParam] = true;
    }

    inline static bool ms_Pressed[0xFF];
    inline static bool ms_WasReleased[0xFF];
    inline static bool ms_WasPressed[0xFF];
