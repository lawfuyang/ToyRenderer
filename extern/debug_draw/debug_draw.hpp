...
struct InternalContext DD_EXPLICIT_CONTEXT_ONLY(: public OpaqueContextType)
{
    ...
    std::atomic<int>   debugStringsCount; // [rlaw]: change to atomic counts
    std::atomic<int>   debugPointsCount;  // [rlaw]: change to atomic counts
    std::atomic<int>   debugLinesCount;   // [rlaw]: change to atomic counts
...
template<typename T>
static void clearDebugQueue(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx,) T * queue, std::atomic<int>& queueCount) // [rlaw]: change to atomic counts
{
...
bool hasPendingDraws(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx))
{
    ...
    std::shared_lock lck{ s_globalContext->sharedMutex };
...
void flush(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx,) const std::int64_t currTimeMillis, const std::uint32_t flags)
{
    ...
    std::unique_lock lck{ s_globalContext->sharedMutex };
